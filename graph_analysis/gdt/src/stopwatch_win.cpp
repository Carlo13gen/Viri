...
#include <time.h>      // for gettimeofday, timeval, timezone
...
long long int stopwatch::_take_the_clock() {
  //tm my_tms;       // the tms structure to extract times.
  struct __timeb64 my_tms;
  //struct timeval  tv;
  //struct timezone tz;
  long int result;  // used only for absolute time
...
double stopwatch::get() {
  if( !_time_pause ) { // the stopwatch is running
    ...
    long long int _now = _take_the_clock();

    // time lapsed between last "start_stopwatch" and now
    //
    long long int diff = _now - _previous_clock_value;
...
double stopwatch::pause()
{
  ...
  {
    _time_pause = true;
    long long int diff = _take_the_clock() - _previous_clock_value;
    _total_time_value += diff;

...
def creaListaArchiToltiEListaRoot(parassite_tree_root, graph):
	...
	def visitaAlbero(graph,nodo,depth):
		...
		if not (len(edges) == 0):
			...
			result = qualeArcoRimuovere(edge1,edge2,nodo);
...
def qualeArcoRimuovere(arco1, arco2, nodo):
	#rimuovo il primo
	return { "nodoRimosso" : arco1 , "nodoRimasto" : arco2 }
...
def label_host_tree_nodes(embedding, graph):
	...
	def label_nodes(current_node, current_edge):
		...
		graph.node[right_child_to_label]["right_child_euristica2"] = True
		graph.node[right_child_to_label]["left_child_euristica2"] = False

		graph.node[left_child_to_label]["left_child_euristica2"] = True
		graph.node[left_child_to_label]["right_child_euristica2"] = False
	...
	graph.node[host_tree_root]["left_child_euristica2"] = False
	graph.node[host_tree_root]["right_child_euristica2"] = False
